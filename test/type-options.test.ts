...
describe("Type options", () => {
  ...
  it("Allows to set type options with toZodSchema() the same as .mongoose()", () => {
    ...
    const zodSchema = toZodgooseSchema(zodObject, {
      typeOptions: {
        username: { ...USERNAME_OPTIONS },
        registered: { ...REGISTERED_OPTIONS },
      },
  ...
  it("Allows to set type options for a specific field with addMognooseTypOptions the same as .mongooseTypeOptions()", () => {
    const USERNAME_OPTIONS = { unique: true };
    const REGISTERED_OPTIONS = { index: true, default: false };
    const zodSchema = toZodgooseSchema(
      z.object({
  ...
  it("Allows to set type options in a sub schema with .mongoose()", () => {
    ...
    expect((Schema.paths.info as any)?.schema.paths.username?.options).toMatchObject(
      USERNAME_OPTIONS,
    );
    expect((Schema.paths.info as any)?.schema.paths.registered?.options).toMatchObject(
      REGISTERED_OPTIONS,
    );
  });
  ...
  it("Merges type options set in .mongooseTypeOptions() with the ones set in .mongoose() in a sub schema", () => {
    ...
    const usernameOpts = (Schema.paths.info as any)?.schema.paths.username?.options;
    expect(usernameOpts).toMatchObject({ ...USERNAME_OPTIONS_1, ...USERNAME_OPTIONS_2 });
    const registeredOpts = (Schema.paths.info as any)?.schema.paths.registered?.options;
    expect(registeredOpts).toMatchObject({ ...REGISTERED_OPTIONS_1, ...REGISTERED_OPTIONS_2 });
  });
  ...
  it("Does not throw if field is .nullable() but required is set to true", () => {
    ...
    expect(() => toMongooseSchema(zodSchema)).not.toThrow(zodgooseError);
  });
  ...
  it("Throws if field is not .optional() nor .nullish() but required is set to function", () => {
    ...
    const zodSchema = z
      .object({
        username: z.string().mongooseTypeOptions({ required: requiredFn }),
      })
      .mongoose();
